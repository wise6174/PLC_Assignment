...
#include <string>
	...
	Galil::Galil() : Functions(nullptr), g(0), ControlParameters{ 0.0, 0.0, 0.0 }, setPoint(0) {
		...
		if (rc != G_NO_ERROR) {
			...
			throw std::runtime_error(std::string("GOpen failed for default address '") +
				"192.168.0.120 -d" + "' with code " + std::to_string(rc));
		}
    };	
	...
	// TODO: complete this function.
	Galil::Galil(EmbeddedFunctions* Funcs, GCStringIn address);	// Constructor with EmbeddedFunciton pre-initialised and passed in.
	// TODO: complete this function.
